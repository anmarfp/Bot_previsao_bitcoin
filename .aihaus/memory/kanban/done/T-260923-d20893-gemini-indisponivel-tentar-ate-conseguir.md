...
- [x] Timeout: Given a chamada ao Gemini estoura o timeout do cliente (httpx.TimeoutException) ou tem outra falha de rede (httpx.TransportError) When gerar_json é chamado Then espera alguns minutos (ESPERA_TIMEOUT) e tenta de novo, sem limite de tentativas; o cliente genai é criado com um timeout explícito (teste).
- [x] Erros permanentes continuam falhando na hora: um código que não é temporário (ex.: 400, 401, 403) é relançado sem nova tentativa, e uma resposta com JSON inválido ou reprovada pelo validador continua limitada a 5 tentativas (teste).
- [x] Sem perda de histórico durante esperas longas: Given uma tarefa (manhã ou noite) fica esperando o Gemini enquanto outra grava no historico_bitcoin.json When a primeira termina Then ela relê o histórico logo antes de alterar e salvar, e a entrada gravada pela outra tarefa é preservada; a noite avalia a mesma entrada que selecionou no início (mesma data e preco_8h) e continua usando o preço capturado antes de chamar o Gemini (teste que simula a gravação concorrente).
- [x] `python -m py_compile robot_bitcoin.py` e `python -m unittest -v test_robot_bitcoin` passam sem rede e sem esperas reais (time.sleep simulado); requirements.txt sem dependências novas.
...
- 2026-09-23: a entrega do agy (b3c6cc7) foi revisada. A lógica está correta e cobre os 5 critérios. Verificado pelo orquestrador: o teste de regressão FALHA no código de c08a04b (ServerError 503 na 5ª chamada) e passa no novo. Corrigido pelo orquestrador: test_concorrencia_manha fazia chamadas reais de rede (CoinGecko e RSS; 2,8 s), agora com mercado e notícias simulados (a suíte roda em 0,09 s); comentários sem acento ou desatualizados. Commit final: 975a58f.
- Mesma classe de falha, fora do escopo e ainda em aberto: se o CoinGecko estiver fora às 22h, a noite falha ao buscar o preço, depois das 3 tentativas do pedir(). Repetir por horas mudaria o preço avaliado. É uma decisão do dono (ex.: usar a vela das 22h da Binance como alternativa).
- Pendência operacional: a previsão de 2026-09-22 ficou sem avaliação no servidor. A recuperação mexe no histórico de produção e exige a aprovação do dono.

## Evidence

Artifact: .aihaus/evidence/T-260923-d20893.json
