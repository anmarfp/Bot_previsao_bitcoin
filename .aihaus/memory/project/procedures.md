...
- Resolved (2026-09-22 23:55 BRT, approved by the owner): the 2026-09-22 prediction was evaluated on the server by a one-off script that runs `verificacao_noite()` with `obter_preco_bitcoin` fixed to the 22:00 BRT Binance candle open (86,457.31), `agora` fixed to 22:00 and Telegram patched. Result: ⚪ SEM VARIAÇÃO (+0.25%). The entry carries an `avaliacao_manual` note. Backup: `historico_bitcoin.json.bak-20260923-antes-avaliacao-2209`. Reuse this pattern (backup, same code path, provenance field, no message) for any future missed evaluation.
- Resolved (2026-09-23, owner said to proceed): `obter_preco_bitcoin` falls back to the Binance BTCUSDT ticker when CoinGecko fails, so `noite` and the 2-hour update no longer depend on one source. `obter_mercado` (morning) still requires CoinGecko. Source: `robot_bitcoin.py` (reviewed 8f76970).
...
- Worktrees have no `.venv`. Use `C:/Users/Marco/Documents/Bot_previsao_bitcoin/.venv/Scripts/python.exe`. Git does not track empty kanban folders, so run `mkdir -p .aihaus/memory/kanban/{backlog,todo,doing,review,done}` before `task.mjs move`, both in a new worktree AND on main (once every task is done, `doing` disappears from main).
